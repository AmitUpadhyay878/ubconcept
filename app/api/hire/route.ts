...
export async function POST(request: Request) {
  try {
    ...
    const { fname,lname,email,company,companyWebsite, details} = body;
    ...
    const mailOptions = {
      from: `<${email}>`, // Sender address (shows sender's name and email)
      to: process.env.RECIPIENT_EMAIL, // Your receiving email address from env variable
      replyTo: email, // So you can reply directly to the sender
      subject: `New Hire-Inquiry Form Submission from ${fname} ${lname}`,
  ...
  <div style="padding: 28px 32px; background-color: #f7fafc;">
    <div style="margin-bottom: 24px;">
      ...
      <div style="background-color: #ffffff; padding: 18px; border-radius: 6px; border: 1px solid #e2e8f0;">
        <p style="margin: 0 0 10px 0; color: #4a5568; font-size: 15px; line-height: 1.6;"><strong style="color: #2d3748; font-weight: 500; min-width: 90px; display: inline-block;">Name:</strong> ${fname} ${lname}</p>
        ...
        <p style="margin: 0; color: #4a5568; font-size: 15px; line-height: 1.6;"><strong style="color: #2d3748; font-weight: 500; min-width: 90px; display: inline-block;">Subject:</strong> New Hire-Inquiry </p>
      </div>
    </div>
    <div>
